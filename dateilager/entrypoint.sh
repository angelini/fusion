...
wait_for_postgres() {
    ...
    until psql "${dburi}?connect_timeout=1" -c '\q' 2> /dev/null; do
        sleep 1
...
write_reset_script() {
    ...
    cat <<EOF > "${file}"
#!/usr/bin/env bash
migrate -path "/etc/dateilager/migrations" -database "${appdb}?sslmode=disable" down -all
migrate -path "/etc/dateilager/migrations" -database "${appdb}?sslmode=disable" up
EOF
    chmod +x "${file}"
}
